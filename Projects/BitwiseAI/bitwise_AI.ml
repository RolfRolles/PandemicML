...
let abstract_add_or_sub initial_carry f_rhs lhs_arr rhs_arr =
  ...
  if lhs_size <> rhs_size then failwith "typechecking prevents this";
...
| _,_ -> failwith "typechecking prevents this" 
...
| _ -> failwith "typechecking prevents this"
...
| Mem(_,_) -> AbsMem(default_abstract_memory)
...
| Binop(e1,((And|Or|Xor|Add|Sub|UMul|EQ|NE) as o) ,e2) -> 
...
| Binop(e1,((Shl|Shr|Sar) as o),e2) ->
 ...
 (match te2 with
  ...
  | _ -> (AbsValue(make_top (Typecheck.type_of_integer_type (Typecheck.typecheck_expr e1))),Binop(e1,o,te2)))
...
| Binop(e1,o,e2) ->
  ...
  (AbsValue(make_top (Typecheck.type_of_integer_type (Typecheck.typecheck_expr expr))),Binop(te1,o,te2))
...
(* Stores and loads are pretty hairy.  We ignore all loads and stores from 
   addresses that are not solely functions of ESP.  If it is a function of
   ESP, replace ESP with a constant value and use that as the address. *)
| Store(m,a,_,s)
| Load(m,a,s)    -> 
  if(match (Irutil.VarSet.elements (Irutil.var_setify (Localopt.extract_uses a))) with 
     | (Variable(i,TypeReg_32))::[] when i = 4 -> true
     | _ -> false)
  then (
    (* This is really stupid, but my PoC works.  Reduce ESP-based addresses to 
       constants. *)
    let h = Hashtbl.create 2 in
    Hashtbl.add h (Variable(4,TypeReg_32)) (Const(0x12345678L, TypeReg_32));
    let ca = Localopt.replace_var_with_expr h a in
    let ca = Localopt.fold_expr_constants ca in
    let ca = match ca with
    | Const(i,TypeReg_32) -> i
    | _ -> failwith ("Expression "^Ppir.ppExpr false a^", supposedly only containing the variable ESP, failed to evaluate to a constant")
    in
    ...
    | Store(_,_,t,_) -> 
      ...
      let make_normalized_store_pairs a s =
        ...
        let rec aux i =
          if i < s
          then (Int64.add a (Int64.of_int i), Array.sub (unwrap_abstract_value at) (i*8) 8)::(aux (i+1))
          else []
        in aux 0
      in
      ...
      List.iter 
        (fun (where,what) -> 
          print_endline ("Storing to "^Int64.to_string where^": "^string_of_abstract_bitvector what))
        store_pairs;
    ...
    | Load(_,_,_)    -> 
      ...
      aux 0;
      print_endline ("Loading from "^Int64.to_string ca^"("^Ppir.ppTypereg s^"): "^string_of_abstract_bitvector outval);
      (AbsValue(outval),Load(tm,a,s))
    | _ -> failwith "impossible")
  else
    (AbsValue(make_top s),expr)
...
let bitwise_abstract_interpret_instr instr =
  ...
  | CJmp(c,t,nt) -> let _,expr = bitwise_abstract_interpret c in 
                   (match expr with
                    ...
                    | Const(_,_) -> failwith "typechecking prevents this"
                    | _ -> CJmp(expr,t,nt))
...
(* Abstract interpret an entire list of instructions, and then clear the 
   context. *)
let bitwise_abstract_interpret_instr_list instrlist =
  Hashtbl.clear varctx;
  let res = List.map bitwise_abstract_interpret_instr instrlist in
  Hashtbl.clear varctx;
  res
