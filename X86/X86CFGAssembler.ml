...
let assemble_channel cfg startea ob =
  let startv = 
    try 
      C.find_vertex cfg startea
    with
      Not_found -> failwith (Printf.sprintf "%08lx: assembler:  can't find staring vertex" startea)
  in
  let ordered_vertex_list = 
    try
      X86CFG.X86CFGBuilder.get_order cfg startv
    with
      Not_found -> failwith "assembler: get_order threw Not_found"
  in
  let assemble_list list =
    let open X86 in
    Array.of_list (List.concat (List.map (fun i -> X86Encode.encode_instruction i) list))
  in
  let make_brick label x86l = Brick(label,assemble_list x86l) in
